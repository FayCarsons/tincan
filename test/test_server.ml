...
(* Sadly, due to Riot's multi-threading and scheduler system,
   testing is difficult for an app like this
   and is going to require more work than I can put in currently
...
let test_custom_events _ =
  ...
  assert_equal
    ...
    ( Chat { init_state with history = [ (init_state.role, "oops") ] },
      Command.Noop );
  assert_equal
    (update (Event.Custom (Utils.Received "hi")) (Chat init_state))
    ( Chat { init_state with history = [ (not_role init_state.role, "hi") ] },
      Command.Noop );
  assert_equal
    (update (Event.Custom Utils.Acknowleged) (Chat init_state))
    (Chat init_state, Command.Noop);
  assert_equal
    (update (Event.Custom (Utils.Connected Riot.Pid.zero))
       (Chat { init_state with connection = Waiting }))
    ( Chat
        {
          init_state with
          history = [ (Host, "Established connection with client") ];
        },
      Command.Noop );
  assert_equal
...
let test_frame _ =
  ...
  let original, updated =
    ...
    | og, Chat updated
      when Option.is_some og.spinner && Option.is_some updated.spinner ->
