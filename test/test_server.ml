...
let test_custom_events _ =
  ...
  assert_equal
    ...
    ( Chat
        { init_state with history = Vec.of_list [ (init_state.role, "oops") ] },
      Command.Noop );
  assert_equal
    ...
    ( Chat
        {
          init_state with
          history = Vec.of_list [ (not_role init_state.role, "hi") ];
        },
      Command.Noop );
  assert_equal
    ...
    ( Chat
        {
          init_state with
          history =
            Vec.of_list [ (Utils.Host, "Established connection with client") ];
        },
      Command.Noop );
  assert_equal
