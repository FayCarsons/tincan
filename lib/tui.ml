...
let validate_uri uri =
  ...
  if Uri.host as_t |> Option.is_some then Ok as_t else Error "Uri must contain a host!"
;;

(** [validate_port port_string] asserts that port is a number and is currently open *)
let validate_port port =
  let exn_message = "Invalid port/Port already in use" in
  let is_open port_num =
    let addr = Unix.ADDR_INET (Unix.inet_addr_loopback, port_num) in
    ...
    | _ -> Error exn_message
  in
  let in_range p = if 0 < p && p <= 65535 then Some p else None in
  int_of_string_opt port
  |> Fun.flip Option.bind in_range
  |> Option.to_result ~none:exn_message
  |> Fun.flip Result.bind is_open
;;

let default_port = 8080
let default_uri = "//0.0.0.0:8080"
...
let validate_config ({ text; role; init_message } as model) =
  let current_text = Text_input.current_text text in
  let validate_host port_str =
    match validate_port port_str with
    | Ok port ->
      Riot.send_by_name ~name:Server.Handler.name @@ StartServer (port, Riot.self ());
  ...
  let validate_client uri =
    ...
    | Ok uri ->
      Riot.send_by_name ~name:Server.Handler.name @@ StartClient (uri, Riot.self ());
  ...
  then (
    let maybe_port =
      if String.length current_text = 0 then string_of_int default_port else current_text
    in
    validate_host maybe_port)
  else (
    let uri = if String.length current_text = 0 then default_uri else current_text in
    validate_client uri)
;;
...
(** [send_client] attempts to send a message when in client mode.
    this acts as a no-op if a message is already pending *)
let send_client ({ role; connection; acknowleged; history; text; _ } as model) =
...
let update_chat ({ role; acknowleged; text; history; spinner; connection } as model)
  ...
  (* Keyboard input *)
  (* Send messsage if we aren't already waiting for one to be acknowledged *)
  | Event.KeyDown (Event.Enter, _) when role = Client -> send_client model, Command.Noop
  ...
  (* Other keys are concatenated onto our text input buffer, if our last message isn't pending *)
  | key ->
    let model =
...
let info_style fmt =
  Spices.(
    default |> italic true |> margin_top 2 |> margin_left 2 |> fg info_color |> build)
    fmt
;;

let error_style =
  Spices.(
    default |> italic true |> margin_top 2 |> margin_left 2 |> fg error_color |> build)
;;
...
let menu_style fmt = Spices.(default |> margin_top 2 |> margin_bottom 2 |> build) fmt
...
let view_menu current_idx =
  ...
  in
  let options =
    List.mapi (fun i (choice, _, _) -> place_cursor i choice) menu |> String.concat "\n"
  in
...
let view_config { text; init_message; role } =
  ...
  | Host ->
    ...
    in
    let err =
      match init_message with
      | Error e -> e
      | _ -> "" |> error_style
    in
    info_style "Please enter the port you'd like to use: " ^ current_port ^ "\n" ^ err
  | Client ->
    let current_uri =
    ...
    let err =
      ...
      | _ -> "" |> error_style
    in
    info_style "Please enter the inet address or host you'd like to connect to: "
    ^ current_uri
    ^ "\n"
    ^ err
;;
